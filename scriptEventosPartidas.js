...
async function appendJson(jsonString, fixtureId) {
    await fs.appendFile('../EventosPartidas.json', `{"fixture_id": ${fixtureId}, "events": ${jsonString}},`, err => {
        if (err) {
...
const options = {
    method: 'GET',
    url: `https://api-football-v1.p.rapidapi.com/v2/events/${partidaAtual}`,
    headers: {
...
function getLineupsFromApi() {
    partidasId.map(async (valor, index) => {
        options.url = `https://api-football-v1.p.rapidapi.com/v2/events/${valor}`;
        console.log(options.url, index)

        const jsonString = await axios.request(options).then(function (response) {
            return JSON.stringify(response.data.api.events)
        }).catch(error => {
